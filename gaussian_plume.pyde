...
def draw():
    ...
    for pstart in range(len(gauss_p_list_start)):

        #r=rd.randint(30,120)
        ra=rd.randint(10,120)
        npieces=rd.randint(20,40)
        xi=gauss_p_list_start[pstart][0]
        yi=gauss_p_list_start[pstart][1]
        #print(xi,yi)
        gauss_p_list=weird_skribble(x=xi,y=yi,r=30,n=npieces,dens=300)
        shift=rd.randint(1,200)
        colug=rd.choice(color_list)
        for p in range(len(gauss_p_list)):
            ...
            point(xi,yi)
            for rs in range(ra):
                reds=[x+rd.randint(0,100) for x in colug]
                #print(reds)
